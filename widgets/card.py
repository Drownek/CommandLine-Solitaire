    ...
    def on_click(self) -> None:
        ...
        if pile:
            if self.is_selected():
                ...
                # Not moving from pile
                    ...
                    self.screen.query_one(StashWaste).unselect_all_cards()
        ...
        if self in foundation.cards:
            ...
            if selected_card_pile:
                ...
                pile_cards = selected_card_pile.cards.copy()
                pile_cards.remove(bottom_card)

                foundation = self.screen.query_one(Foundation)
                foundation_cards: list[Card | None] = foundation.cards.copy()
                foundation_cards[foundation_cards.index(self)] = bottom_card
                foundation.cards = foundation_cards

                bottom_card.make_unselected()

                if pile_cards:
                    pile_cards[-1].unhide()

                selected_card_pile.cards = pile_cards
